...
import 'package:newapp/services/quote_service.dart';
import 'package:newapp/services/background_removal_service.dart';
import 'package:newapp/services/category_service.dart';
import 'package:newapp/services/payment_service.dart';
...
class HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
  ...
  List<Map<String, dynamic>> userProfilePhotos =
      []; // List to store multiple profile photos (now as maps)
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (state == AppLifecycleState.resumed) {
      print(
        '🔄 [LIFECYCLE] App resumed - checking payment status and refreshing subscription',
      );
  ...
  void _startPaymentCheckTimer() {
    ...
    print(
      '🚀 [TIMER] Payment check timer started - will check every 10 seconds',
    );
  }
    ...
    try {
      ...
      if (userData != null) {
        setState(() {
          ...
          userUsageType =
              userData['usageType'] ??
              'Personal'; // Default to Personal if not set
    ...
    try {
      ...
      if (userData != null) {
        ...
        setState(() {
          ...
          subscriptionAmount = (userData['subscriptionAmount'] ?? 0.0)
              .toDouble();
          ...
          if (userData['subscriptionEndDate'] != null) {
            subscriptionEndDate = (userData['subscriptionEndDate'] as Timestamp)
                .toDate();
          }
        });
        print(
          '✅ [HOME] User subscription refreshed: $oldSubscription → $userSubscription',
        );
      ...
      children: [
        ...
        Row(
          children: [
            ...
            SizedBox(width: 4),
            Text(
              subscriptionPlanTitle.isNotEmpty
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            _buildDetailRow(
              'Plan',
              (userSubscription.toLowerCase() == 'paid' ||
                      userSubscription.toLowerCase() == 'freemium')
                  ? 'Premium'
                  : (userSubscription.toLowerCase() == 'free'
                        ? 'Free'
                        : userSubscription),
            ),
            _buildDetailRow(
            ...
            if (subscriptionStartDate != null)
              _buildDetailRow(
                'Start Date',
                _formatFullDate(subscriptionStartDate!),
              ),
            if (subscriptionEndDate != null)
              _buildDetailRow(
                'End Date',
                _formatFullDate(subscriptionEndDate!),
              ),
            if (subscriptionEndDate != null) ...[
              SizedBox(height: 12),
              Container(
  ...
  Future<void> _checkPaymentStatus() async {
    try {
      ...
      print(
        '🧾 [PAYMENT] Loaded last payment ID from SharedPreferences: $paymentId',
      );
      ...
      print(
        '📤 [PAYMENT] (Frontend) POST /checkPaymentStatus with paymentId: $paymentId',
      );
      final response = await http.post(
        Uri.parse(
          'https://us-central1-prime-status-1db09.cloudfunctions.net/checkPaymentStatus',
        ),
      ...
      if (response.statusCode == 200) {
        print(
          '📥 [PAYMENT] (Frontend) Response: ${response.statusCode} ${response.body}',
        );
  ...
  Future<void> _checkPaymentStatusSilent() async {
    try {
      ...
      if (response.statusCode == 200) {
        print(
          '📥 [TIMER] Silent check response: ${response.statusCode} ${response.body}',
        );
        final responseData = jsonDecode(response.body);
        final success = responseData['success'] as bool? ?? false;
        final status = responseData['status'] as String? ?? 'unknown';
  ...
  Future<void> _updateSubscriptionToPaid(String paymentId) async {
    try {
      ...
      if (paymentAttempts.docs.isNotEmpty) {
        ...
        print(
          '❌ [PAYMENT] No payment attempt found for payment ID: $paymentId',
        );
      }
    } catch (e) {
                  ...
                  children: [
                    ...
                    FutureBuilder<SharedPreferences>(
                      ...
                      builder: (context, snapshot) {
                        if (snapshot.hasData) {
                          return Text(
                            snapshot.data!.getString('last_payment_id') ??
                                'Not found',
                          );
                        }
                        return Text('Loading...');
  ...
  void _showShareLinkDialog() async {
    try {
      ...
      if (shareLink.isEmpty) {
        shareLink =
            'https://play.google.com/store/apps/details?id=com.primestatus.app';
      }
...
''';
  ...
  Future<void> _fetchUserProfilePhotos() async {
    ...
    try {
      ...
      final photosSnapshot = await FirebaseFirestore.instance
          ...
          .get();
      print(
        '_fetchUserProfilePhotos: Found ${photosSnapshot.docs.length} photos',
      );
      setState(() {
      ...
      print(
        '_fetchUserProfilePhotos: Updated state with ${userProfilePhotos.length} photos',
      );
    } catch (e) {
                    ...
                    child: ClipRRect(
                      ...
                      child: Crop(
                        ...
                        cornerDotBuilder: (size, edgeAlignment) =>
                            const DotControl(color: Colors.deepOrange),
                        interactive: true,
                        fixCropRect: true,
                        radius: 20,
  ...
  void _selectCategoryFromSearch(String categoryName) {
    ...
    final englishCategoryName = _getEnglishCategoryNameFromDisplay(
      categoryName,
    );
  ...
  Future<void> _addProfilePhotoToGallery(
    String photoUrl, {
    String? photoUrlNoBg,
  }) async {
    ...
    try {
      await FirebaseFirestore.instance
          ...
          .add({
            ...
            'isActive': false,
            'withoutBackground':
                photoUrlNoBg != null, // Mark as processed if available
          });
    } catch (e) {
                            ...
                            onTap: () async {
                              if (isLoggedIn) {
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: Text(
                                      'Please sign in to change usage type',
                                    ),
                                  ),
                                );
                              }
            ...
            itemBuilder: (context, index) {
              ...
              return Container(
                width:
                    ...
                    5.5, // Show 5 categories at a time
  ...
  Widget _buildAdminCategoryChip(
    String label,
    bool isSelected, {
    IconData? icon,
  }) {
      ...
      return SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            CommonWidgets.buildProfileOption(
              'Share App',
              Icons.share,
              () => _showShareAppDialog(),
            ),
                  ...
                  child: Column(
                    children: [
                      _buildUserDataCard(
                        ...
                        Icons.person,
                        onTap: () =>
                            _showEditFieldDialog('Name', userName, 'name'),
                      ),
                      _buildUserDataCard(
                        'Designation',
                        userDesignation,
  ...
  void _showContactUsDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'For any queries, email us at support@primestatusapp.com',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
  ...
  void _showMultiFontDialog() {
    final fontList = [
      {'name': 'BarahaUnicode', 'file': 'BarahaUnicode.ttf'},
      {
        ...
        'file': 'NotoSansKannada-VariableFont_wdth,wght.ttf',
      },
          ...
          child: Scrollbar(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final fontFamily =
                    fontName; // Font family should match pubspec.yaml
                return Padding(
