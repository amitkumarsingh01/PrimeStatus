...
import 'package:newapp/screens/AllSubscription.dart';
...
class FullscreenVideoControllerManager {
  static final FullscreenVideoControllerManager _instance =
...
class _FullscreenPostViewerState extends State<FullscreenPostViewer> {
                  ...
                  child: _filteredPosts.isEmpty
                      ...
                      : PageView.builder(
                          ...
                          onPageChanged: (index) {
                            ...
                            FullscreenVideoControllerManager()
                                .pauseAllControllers();
                          },
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          _isProcessingShare
                              ? 'Processing post for sharing...'
                              : 'Processing post for download...',
                          style: TextStyle(
                            fontSize: 16,
  ...
  Future<void> _shareVideoWithOverlays(
    String videoUrl,
    ...
    try {
      ...
      processedFilePath =
          await LocalMediaProcessingService.processVideoWithOverlays(
            videoUrl: videoUrl,
            post: post,
            userUsageType: widget.userUsageType,
            userName: widget.userName,
  ...
  Future<void> _shareToWhatsApp(
    ...
    try {
      ...
      final String? processedFilePath =
          await LocalMediaProcessingService.processImageWithOverlays(
            imageUrl: imageUrl,
            post: post,
            userUsageType: widget.userUsageType,
            userName: widget.userName,
  ...
  Future<void> _downloadImageWithOverlays(
    String imageUrl,
    Map<String, dynamic> post,
  ) async {
    try {
        ...
        if (hasPermission) {
          ...
          if (downloadsDir != null) {
            final String fileName =
                'PrimeStatus_WithOverlays_${DateTime.now().millisecondsSinceEpoch}.png';
  ...
  Future<void> _downloadImageFromUrl(
    String imageUrl,
    Map<String, dynamic> post,
  ) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (hasPermission) {
          ...
          if (downloadsDir != null) {
            ...
            final String fileName =
                'PrimeStatus_${DateTime.now().millisecondsSinceEpoch}.$fileExtension';
            final String filePath = '${downloadsDir.path}/$fileName';
  ...
  Future<void> _downloadVideoWithOverlays(
    String videoUrl,
    Map<String, dynamic> post,
  ) async {
    try {
        ...
        if (hasPermission) {
          ...
          if (downloadsDir != null) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                  '${processingMethod == 'full_video' ? 'Video' : 'Thumbnail'} saved successfully!',
                ),
                duration: Duration(seconds: 3),
                action: SnackBarAction(
                  label: 'Share',
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Error downloading video: $e')));
    }
  }

  Future<Uint8List?> _captureImageWithOverlays(
    String imageUrl,
    Map<String, dynamic> post,
  ) async {
    try {
      ...
      // Capture the widget
      final Uint8List? capturedBytes = await _screenshotController
          .captureFromWidget(
            Material(color: Colors.transparent, child: imageWithOverlays),
      ...
      if (capturedImage.height > cropHeight) {
        ...
        final int cropStartY = removedTop.clamp(
          0,
          capturedImage.height - cropHeight,
        );
  ...
  Future<Uint8List?> _createSimpleImageWithOverlays(
    String imageUrl,
    Map<String, dynamic> post,
  ) async {
    try {
        ...
        content: Container(
          ...
          child: Column(
            children: [
              ...
              Expanded(child: _buildProfilePhotoGallery()),
              SizedBox(height: 16),
  ...
  Future<void> _fetchUserProfilePhotos() async {
    ...
    try {
      ...
      final photosSnapshot = await FirebaseFirestore.instance
          ...
          .get();
      setState(() {
        _userProfilePhotos = photosSnapshot.docs
            .map((doc) => doc.data())
            .toList();
        _activeProfilePhotoUrl = widget.userProfilePhotoUrl;
      });
  ...
  Widget _buildProfilePhotoGallery() {
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final isActive =
            photoUrl == _activeProfilePhotoUrl ||
            photoUrlNoBg == _activeProfilePhotoUrl;
        return Row(
          children: [
            if (photoUrl != null)
              Expanded(
                child: GestureDetector(
                  ...
                  child: Container(
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(
                        color: isActive && _activeProfilePhotoUrl == photoUrl
                            ? Colors.green
                            : Colors.grey.shade300,
                        width: isActive && _activeProfilePhotoUrl == photoUrl
                            ? 2
                            : 1,
                      ),
                    ),
                      ...
                      child: Stack(
                        children: [
                          CachedNetworkImage(
                            ...
                            memCacheWidth:
                                150, // Optimize for gallery thumbnails
                            memCacheHeight: 150,
                ...
                child: GestureDetector(
                  ...
                  child: Container(
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(
                        color:
                            isActive && _activeProfilePhotoUrl == photoUrlNoBg
                            ? Colors.green
                            : Colors.grey.shade300,
                        width:
                            isActive && _activeProfilePhotoUrl == photoUrlNoBg
                    ...
                    child: ClipRRect(
                      ...
                      child: Stack(
                        children: [
                          ...
                          if (isActive &&
                              _activeProfilePhotoUrl == photoUrlNoBg)
                            Positioned(
                              top: 4,
                              right: 4,
                              child: Container(
  ...
  Future<void> _testVideoProcessing(
    String videoUrl,
    Map<String, dynamic> post,
  ) async {
    try {
      ...
      final String? thumbnailPath =
          await VideoProcessingService.createVideoThumbnailWithOverlay(
            videoUrl: videoUrl,
            post: post,
            userUsageType: widget.userUsageType,
            userName: widget.userName,
...
class AdminPostFullScreenCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final frameSize =
        forceFrameSize ?? post['frameSize'] ?? {'width': 1080, 'height': 1920};
          ...
          children: [
            LayoutBuilder(
              builder: (context, constraints) {
                ...
                final double textXBase =
                    (textSettings['x'] ?? 50) / 100 * width;
                final double textY = (textSettings['y'] ?? 90) / 100 * height;
                final double profileX =
                    (profileSettings['x'] ?? 20) / 100 * width;
                final double profileY =
                    (profileSettings['y'] ?? 20) / 100 * height;
                final double profileSize = (profileSettings['size'] ?? 80)
                    .toDouble();
                final double addressXBase =
                    (addressSettings['x'] ?? 50) / 100 * width;
                final double addressY =
                    (addressSettings['y'] ?? 80) / 100 * height;
                final double phoneX = (phoneSettings['x'] ?? 50) / 100 * width;
                final double phoneY = (phoneSettings['y'] ?? 85) / 100 * height;
                final double textX = userName.length > 15
                    ? textXBase + 30
                    : textXBase;
                final double addressX = userAddress.length > 15
                    ? addressXBase + 45
                    : addressXBase;
                ...
                return SizedBox(
                  ...
                  child: Stack(
                    children: [
                        ...
                        Positioned(
                          ...
                          child: Transform.translate(
                            ...
                            child: Container(
                              ...
                              decoration:
                                  addressSettings['hasBackground'] == true
                                  ? BoxDecoration(
                                      color: _parseColor(
                                        addressSettings['backgroundColor'] ??
                                            '#000000',
                                      ),
                                      borderRadius: BorderRadius.circular(8),
                                    )
                              ...
                              child: Text(
                                ...
                                style: TextStyle(
                                  fontFamily: getFontFamily(
                                    addressSettings['font'],
                                  ),
                                  fontSize:
                                      ((addressSettings['fontSize'] ?? 18)
                                          .toDouble()) *
                                      (userAddress.length > 15 ? 0.9 : 1.0),
                                  color: _parseColor(
                                    addressSettings['color'] ?? '#ffffff',
                                  ),
                                  fontWeight: FontWeight.bold,
                                ),
                              ),
                      ...
                      if (userUsageType == 'Business' &&
                        ...
                        Positioned(
                          ...
                          child: Transform.translate(
                            offset: Offset(
                              -0.5 *
                                  (phoneSettings['fontSize'] ?? 18) *
                                  (userPhoneNumber.length / 2),
                              -20,
                            ),
                            child: Container(
                              padding: const EdgeInsets.symmetric(
                              ...
                              child: Text(
                                ...
                                style: TextStyle(
                                  ...
                                  fontSize:
                                      ((phoneSettings['fontSize'] ?? 18)
                                          .toDouble()) *
                                      (userPhoneNumber.length > 15 ? 0.9 : 1.0),
                                  color: _parseColor(
                                    phoneSettings['color'] ?? '#ffffff',
                                  ),
                                  fontWeight: FontWeight.bold,
                                ),
                              ),
                      ...
                      if (profileSettings['enabled'] == true &&
                          userProfilePhotoUrl != null &&
                          userProfilePhotoUrl!.isNotEmpty)
                        Positioned(
...
class _Base64VideoPlayerState extends State<_Base64VideoPlayer> {
  ...
  void initState() {
    super.initState();
    _controllerKey =
        'fullscreen_base64_${DateTime.now().millisecondsSinceEpoch}_${widget.bytes.hashCode}';
    _controller = VideoPlayerController.networkUrl(
      Uri.parse('data:video/mp4;base64,${base64Encode(widget.bytes)}'),
    );
    FullscreenVideoControllerManager().registerController(
      _controllerKey,
      _controller,
    );
...
class _NetworkVideoPlayerState extends State<_NetworkVideoPlayer> {
  ...
  void initState() {
    super.initState();
    _controllerKey =
        'fullscreen_network_${DateTime.now().millisecondsSinceEpoch}_${widget.url.hashCode}';
